...
func (h *BindHandle) Update(fullLoad bool) (err error) {
	...
	rows, _, err := exec.ExecRestrictedSQL(metrics.InternalContext("BindInfo"), nil, `SELECT original_sql, bind_sql, default_db, status, create_time, update_time, charset, collation, source
...
func (h *BindHandle) CreateBindRecord(sctx sessionctx.Context, record *BindRecord) (err error) {
	...
	_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), "BEGIN PESSIMISTIC")
	if err != nil {
		return
	}
	...
	defer func() {
		if err != nil {
			_, err1 := exec.ExecuteInternal(metrics.InternalContext("BindInfo"), "ROLLBACK")
			terror.Log(err1)
			return
		}

		_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), "COMMIT")
		if err != nil {
			return
		}
	...
	updateTs := now.String()
	_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), `UPDATE mysql.bind_info SET status = %?, update_time = %? WHERE original_sql = %? AND update_time < %?`,
	...
	for i := range record.Bindings {
		...
		_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), `INSERT INTO mysql.bind_info VALUES (%?,%?, %?, %?, %?, %?, %?, %?, %?)`,
			record.OriginalSQL,
...
func (h *BindHandle) AddBindRecord(sctx sessionctx.Context, record *BindRecord) (err error) {
	...
	if duplicateBinding != nil {
		_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), `DELETE FROM mysql.bind_info WHERE original_sql = %? AND bind_sql = %?`, record.OriginalSQL, duplicateBinding.BindSQL)
		if err != nil {
			return err
		}
	}
...
func (h *BindHandle) DropBindRecord(originalSQL, db string, binding *Binding) (err error) {
	...
	if binding == nil {
		_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), `UPDATE mysql.bind_info SET status = %?, update_time = %? WHERE original_sql = %? AND update_time < %? AND status != %?`,
			deleted, updateTs, originalSQL, updateTs, deleted)
	} else {
		_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), `UPDATE mysql.bind_info SET status = %?, update_time = %? WHERE original_sql = %? AND update_time < %? AND bind_sql = %? and status != %?`,
...
func (h *BindHandle) SetBindRecordStatus(originalSQL string, binding *Binding, newStatus string) (ok bool, err error) {
	...
	if binding == nil {
		_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), `UPDATE mysql.bind_info SET status = %?, update_time = %? WHERE original_sql = %? AND update_time < %? AND status IN (%?, %?)`,
			newStatus, updateTsStr, originalSQL, updateTsStr, oldStatus0, oldStatus1)
	} else {
		_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), `UPDATE mysql.bind_info SET status = %?, update_time = %? WHERE original_sql = %? AND update_time < %? AND bind_sql = %? AND status IN (%?, %?)`,
...
func (h *BindHandle) GCBindRecord() (err error) {
	...
	_, err = exec.ExecuteInternal(metrics.InternalContext("BindInfo"), `DELETE FROM mysql.bind_info WHERE status = 'deleted' and update_time < %?`, updateTimeStr)
	return err
}
...
func (h *BindHandle) lockBindInfoTable() error {
	...
	exec, _ := h.sctx.Context.(sqlexec.SQLExecutor)
	_, err := exec.ExecuteInternal(metrics.InternalContext("BindInfo"), h.LockBindInfoSQL())
	return err
}
...
func (h *BindHandle) extractCaptureFilterFromStorage() (filter *captureFilter) {
	...
	// No need to acquire the session context lock for ExecRestrictedSQL, it
	// uses another background session.
	rows, _, err := exec.ExecRestrictedSQL(metrics.InternalContext("BindInfo"), nil, `SELECT filter_type, filter_value FROM mysql.capture_plan_baselines_blacklist order by filter_type`)
	if err != nil {
...
func getHintsForSQL(sctx sessionctx.Context, sql string) (string, error) {
	...
	rs, err := sctx.(sqlexec.SQLExecutor).ExecuteInternal(metrics.InternalContext("BindInfo"), fmt.Sprintf("EXPLAIN FORMAT='hint' %s", sql))
	...
	chk := rs.NewChunk(nil)
	err = rs.Next(metrics.InternalContext("BindInfo"), chk)
	if err != nil {
		return "", err
	}
...
func getEvolveParameters(ctx sessionctx.Context) (time.Duration, time.Time, time.Time, error) {
	rows, _, err := ctx.(sqlexec.RestrictedSQLExecutor).ExecRestrictedSQL(
		metrics.InternalContext("BindInfo"),
		nil,
...
func (h *BindHandle) getRunningDuration(sctx sessionctx.Context, db, sql string, maxTime time.Duration) (time.Duration, error) {
	ctx := metrics.InternalContext("BindInfo")
	if db != "" {
